...
public class DataUtils {
     ...
     * @return
     */
    public static Boolean isStringValueExist(Activity activity, String value, String name, Boolean show_failure) {
        Boolean exist = true;

        if (value== null) exist = false;
        else if (value.trim().equals("")) exist = false;
        else if (value.trim().equalsIgnoreCase("null")) exist = false;
        else if (value.trim().equalsIgnoreCase(ValueUtils.NOT_DEFINED)) exist = false;


        if(show_failure && (!exist))
            DisplayUtils.showMessageError(activity, name+" value is not valid");
